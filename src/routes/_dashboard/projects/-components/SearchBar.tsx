...
import {
  ...
  filterList,
} from '@/store/projectsFilterStore.tsx';
import {Fragment} from 'react';

export function SearchBar() {
  const {projectFilters, removeProjectFilter, addProjectFilter} =
    useProjectFilterStore((s) => ({
          ...
          <Popover.Group className="-mx-4 flex items-center divide-x divide-gray-200">
            <Popover className="relative inline-block px-4 text-left">
              <Popover.Button className="group inline-flex justify-center text-sm font-medium text-gray-700 hover:text-gray-900 focus:outline-none">
                <span className="rounded bg-gray-200 px-1.5 py-0.5 text-xs font-semibold tabular-nums text-gray-700">
                      ...
                      <div
                        ...
                        <input
                          ...
                          defaultChecked={
                            !!projectFilters.find(
                              (projectFilter) =>
                                projectFilter.mode === filterData.mode,
                            )
                          }
                          onChange={(e) => {
                            if (!e.target.checked) {
